        ...
        <tbody id="tableBody" class="divide-y divide-slate-100">
            ...
            <tr class="group hover:bg-slate-50/50 transition-all duration-300 border-l-4 border-l-transparent hover:border-l-[#FF6900]">
                ...
                <td class="py-5 px-6">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-md bg-[#1d293d] flex items-center justify-center text-white shadow-lg">
                            <span class="text-[0.7rem] font-black"><?php echo e(mb_substr($name, 0, 1)); ?></span>
                        </div>
                        <div class="flex flex-col">
                            <span class="text-[0.85rem] font-black text-slate-900 tracking-tight"><?php echo e($name); ?></span>
                            <span class="text-[0.6rem] text-slate-400 font-bold uppercase tracking-wider"><?php echo e($details['phone'] ?? 'NO PHONE'); ?></span>
                        </div>
                    </div>
                </td>
                <td class="py-5 px-6">
                    <div class="flex items-center gap-4">
                        <div class="flex flex-col">
                            <span class="text-[0.85rem] font-black text-slate-900 tracking-tighter"><?php echo e($details['year'] ?? ''); ?> <?php echo e($details['make'] ?? 'Unknown'); ?></span>
                            <span class="text-[0.7rem] text-slate-500 font-bold italic"><?php echo e($details['model'] ?? 'N/A'); ?></span>
                        </div>
                    </div>
                </td>
                <td class="py-5 px-6">
                    <div class="flex flex-col gap-1">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[0.6rem] font-black uppercase tracking-widest text-slate-400"><?php echo e($condition); ?></span>
                        </div>
                    </div>
                </td>
                <td class="py-5 px-6">
                    <div class="flex flex-col">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <span class="text-[0.8rem] font-black text-slate-900 leading-none"><?php echo e($appDate); ?></span>
                        </div>
                        <div class="flex items-center gap-2">
                            <?php if($isHome): ?>
                            ...
                            <?php endif; ?>
                            <span class="text-[0.6rem] text-slate-400 font-bold uppercase tracking-widest"><?php echo e($appTime ?: 'ASAP'); ?></span>
                        </div>
                    </div>
                </td>
                <td class="py-5 px-6 text-center">
                    ...
                    ?>
                    <span class="px-4 py-1.5 rounded-md text-[0.65rem] font-black uppercase tracking-widest border shadow-sm <?php echo e($col); ?>">
                ...
                <td class="py-5 px-8 text-right">
                    <div class="flex items-center justify-end gap-3">
                        <?php if($lead->status === 'new' || $lead->status === 'pending' || $lead->status === 'Active'): ?>
                            <button onclick="confirmLead(<?php echo e($lead->id); ?>)" title="Confirm & Schedule" class="w-10 h-10 rounded-md bg-orange-50 border border-orange-100 text-[#FF6900] flex items-center justify-center hover:bg-[#FF6900] hover:text-white transition-all shadow-sm active:scale-95 group">
                                <i data-lucide="calendar-check" class="w-4.5 h-4.5 transition-transform group-hover:scale-110"></i>
                            </button>
                        <?php endif; ?>
                        <button onclick="viewLead(<?php echo e($lead->id); ?>)" title="Open Node" class="w-10 h-10 rounded-md bg-white border border-slate-200 text-slate-400 flex items-center justify-center hover:bg-[#1d293d] hover:text-white transition-all shadow-sm active:scale-95 group">
                            <i data-lucide="eye" class="w-4.5 h-4.5 transition-transform group-hover:scale-110"></i>
                        </button>
                        <a href="https://wa.me/<?php echo e(preg_replace('/[^0-9]/', '', $details['phone'] ?? '')); ?>" target="_blank" class="w-10 h-10 rounded-md bg-emerald-50 border border-emerald-100 text-emerald-600 flex items-center justify-center hover:bg-emerald-600 hover:text-white transition-all shadow-sm active:scale-95 group">
                            <i data-lucide="message-circle" class="w-4.5 h-4.5 transition-transform group-hover:scale-110"></i>
                        </a>
                        <button onclick="deleteLead(<?php echo e($lead->id); ?>)" class="w-10 h-10 rounded-md bg-white border border-red-100 text-red-400 flex items-center justify-center hover:bg-red-600 hover:text-white transition-all shadow-sm active:scale-95 group">
...
<?php /**PATH D:\auction_app\resources\views/admin/leads/_table.blade.php ENDPATH**/ ?>