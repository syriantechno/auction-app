...
<?php $__env->startSection('title', 'Vehicle Asset Inventory'); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin-header','data' => ['icon' => 'car','title' => 'Vehicle','highlight' => 'Inventory','dot' => 'amber','subtitle' => 'All registered cars & stock']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['icon' => 'car','title' => 'Vehicle','highlight' => 'Inventory','dot' => 'amber','subtitle' => 'All registered cars & stock']); ?>
         <?php $__env->slot('actions', null, []); ?> 
            <div class="flex items-center gap-2 bg-white px-5 py-2.5 rounded-lg border border-slate-100 shadow-sm">
                ...
                <span id="totalCounter" class="text-xl font-black text-[#031629] tabular-nums"><?php echo e(\App\Models\Car::count()); ?></span>
            </div>
...
<?php endif; ?>
    ...
    <div class="bg-white p-5 rounded-lg border border-slate-200 shadow-sm">
        <form id="filterForm" action="<?php echo e(route('admin.cars.index')); ?>" method="GET" class="flex flex-wrap items-end gap-3">
            <div class="flex-1 min-w-[220px]">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" id="searchInput" value="<?php echo e(request('search')); ?>" placeholder="Search by make or model..." class="w-full h-[44px] bg-slate-50 border border-slate-300 rounded-md pl-11 pr-4 py-2 text-[0.9rem] font-normal text-slate-700 outline-none focus:bg-white focus:border-orange-500/40 transition-all shadow-sm">
                </div>
            </div>
            <div class="w-48">
                ...
                <div class="relative">
                    <select name="make" id="makeFilter" class="w-full h-[44px] bg-slate-50 border border-slate-300 rounded-md px-4 py-2 text-[0.9rem] font-normal text-slate-700 appearance-none outline-none focus:bg-white focus:border-orange-500/40 transition-all">
                        ...
                        <?php $__currentLoopData = ($makeOptions ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $make): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <div class="w-32">
                ...
                <div class="relative">
                    <select name="year" id="yearFilter" class="w-full h-[44px] bg-slate-50 border border-slate-300 rounded-md px-4 py-2 text-[0.9rem] font-normal text-slate-700 appearance-none outline-none focus:bg-white focus:border-orange-500/40 transition-all">
                        ...
                        <?php $__currentLoopData = \App\Models\Car::distinct()->orderBy('year', 'desc')->pluck('year'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $year): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <tr class="group hover:bg-slate-50/50 transition-all duration-200 border-l-4 border-l-transparent hover:border-l-orange-500">
                        <td class="py-3 px-8">
                            <div class="w-11 h-11 rounded-md overflow-hidden bg-white border border-slate-200 shadow-sm flex items-center justify-center mx-auto transition-all group-hover:scale-110 duration-300 p-2">
                                <img src="<?php echo e($car->logo_url); ?>" class="w-full h-auto max-h-full object-contain group-hover:drop-shadow-sm transition-all" alt="<?php echo e($car->make); ?>">
                            </div>
                        </td>
                        <td class="py-3 px-6">
                            <div class="flex items-center gap-4">
                                <span class="text-[0.95rem] font-normal text-slate-700 tracking-tight leading-none group-hover:text-orange-600 transition-colors"><?php echo e($car->brand?->name ?? $car->make); ?> <?php echo e($car->carModel?->name ?? $car->model); ?></span>
                                <span class="bg-slate-50 text-slate-400 px-2.5 py-1 rounded-lg text-[0.55rem] font-mono font-medium uppercase border border-slate-200">#<?php echo e($car->id); ?></span>
                            </div>
                        </td>
                        <td class="py-3 px-6 text-center">
                            <span class="inline-block px-3.5 py-1 bg-white border border-slate-200 rounded-full text-[0.7rem] font-medium text-slate-600"><?php echo e($car->year); ?></span>
                        </td>
                        <td class="py-3 px-8 text-right">
                            <div class="flex items-center justify-end gap-2.5">
                                <button onclick="viewCarDetail(<?php echo e($car->id); ?>, <?php echo json_encode($car->brand?->name ?? $car->make, 15, 512) ?>, <?php echo json_encode($car->carModel?->name ?? $car->model, 15, 512) ?>, <?php echo e($car->year); ?>, <?php echo json_encode($car->image_url, 15, 512) ?>)" title="Inspector" class="w-9 h-9 rounded-md bg-white text-slate-400 flex items-center justify-center hover:bg-emerald-600 hover:text-white transition-all shadow-md border border-slate-200 active:scale-95">
                                    <i data-lucide="eye" class="w-4.5 h-4.5"></i>
                                </button>
                                <button onclick="editCar(<?php echo e($car->id); ?>, <?php echo json_encode($car->brand?->name ?? $car->make, 15, 512) ?>, <?php echo json_encode($car->carModel?->name ?? $car->model, 15, 512) ?>, <?php echo e($car->year); ?>)" title="Edit Row" class="w-9 h-9 rounded-md bg-white text-slate-400 flex items-center justify-center hover:bg-slate-800 hover:text-white transition-all shadow-md border border-slate-200 active:scale-95">
                                    <i data-lucide="edit-3" class="w-4.5 h-4.5"></i>
                                </button>
                                <button onclick="deleteCar(<?php echo e($car->id); ?>)" title="Purge" class="w-9 h-9 rounded-md bg-white text-red-400 flex items-center justify-center hover:bg-red-600 hover:text-white transition-all shadow-md border border-red-50 active:scale-95">
        ...
        <div class="p-8 space-y-6">
            <form id="ajaxCarForm" class="space-y-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" id="inputYear" name="year" required min="1950" max="<?php echo e(now()->year + 1); ?>" class="w-full bg-slate-50 border border-slate-300 rounded-md px-5 py-3.5 text-[0.9rem] font-normal text-slate-700 outline-none focus:bg-white focus:border-orange-500 transition-all shadow-inner">
                </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('ajaxCarForm').addEventListener('submit', async function(e) {
            ...
            const url = id ? `/admin/cars/catalog/${id}` : "<?php echo e(route('admin.car-catalog.store')); ?>";
...
<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH F:\auction_app\resources\views/admin/cars/index.blade.php ENDPATH**/ ?>