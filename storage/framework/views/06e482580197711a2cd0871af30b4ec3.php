...
<?php $__env->startSection('title', ($auction->car->year ?? '') . ' ' . ($auction->car->make ?? '') . ' ' . ($auction->car->model ?? '') . ' - UniteCar'); ?>
...
<script type="application/ld+json">
{
  ...
  "@type": "Product",
  "name": "<?php echo e(($auction->car->year ?? '') . ' ' . ($auction->car->make ?? '') . ' ' . ($auction->car->model ?? '')); ?>",
  "image": [
    "<?php echo e(url($auctionImage)); ?>"
  ],
  "description": "<?php echo e($auction->car->latestInspection->expert_summary ?? 'Premium ' . ($auction->car->make ?? '') . ' car available for auction on Motor Bazar. Certified technical audit included.'); ?>",
  "sku": "AUCTION-<?php echo e($auction->id); ?>",
  "mpn": "<?php echo e($auction->car->vin ?? 'N/A'); ?>",
  "brand": {
    "@type": "Brand",
    "name": "<?php echo e($auction->car->make ?? 'Unknown'); ?>"
  },
  <?php if($auction->car->latestInspection): ?>
  "review": {
    "@type": "Review",
    "reviewRating": {
      "@type": "Rating",
      "ratingValue": "<?php echo e(($auction->car->latestInspection->overall_score ?? 0) / 20); ?>",
      "bestRating": "5"
    },
    "author": {
      "@type": "Organization",
  ...
  "offers": {
    ...
    "priceCurrency": "USD",
    "price": "<?php echo e($auction->current_price ?? $auction->initial_price); ?>",
    "priceValidUntil": "<?php echo e($auction->end_at?->format('Y-m-d')); ?>",
    "itemCondition": "https://schema.org/UsedCondition",
    "availability": "https://schema.org/<?php echo e($auction->status === 'active' ? 'InStock' : 'PreOrder'); ?>",
    "seller": {
      "@type": "Organization",
      "name": "Motor Bazar"
    }
...
<script type="application/ld+json">
{
  ...
  "itemListElement": [
    ...
    },
    {
      ...
      "position": 2,
      "name": "Auctions",
      "item": "<?php echo e(route('auctions.index')); ?>"
    },
    {
      "@type": "ListItem",
      "position": 3,
      "name": "<?php echo e($auction->car->make ?? 'Car'); ?>",
      "item": "<?php echo e(route('auctions.index', ['make' => $auction->car->make ?? ''])); ?>"
    }
  ]
}
</script>
...
<div class="pt-32 pb-20 px-6 lg:px-12 max-w-[1440px] mx-auto min-h-screen">
    ...
    <div class="mb-10">
        <nav class="flex items-center gap-2 text-[0.8rem] text-gray-400 font-semibold mb-3">
            ...
            <span><?php echo e($auction->car->make ?? 'Ford'); ?></span> <i data-lucide="chevron-right" class="w-3.5"></i>
        ...
        <div class="flex items-center gap-5">
            ...
            <h1 class="text-5xl font-black tracking-tight text-[#2a3547] capitalize">
                <?php echo e($auction->car->year ?? ''); ?> <?php echo e($auction->car->make ?? 'Ford'); ?> <?php echo e($auction->car->model ?? ''); ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div class="col-span-1 hidden lg:flex flex-col">
            <div class="thumb-btn active">
                <img src="<?php echo e($auctionThumbImages[0]); ?>" alt="Auction car image 1">
            </div>
            <div class="thumb-btn">
                <img src="<?php echo e($auctionThumbImages[1]); ?>" class="opacity-50 grayscale" alt="Auction car image 2">
            </div>
            <div class="thumb-btn">
                <img src="<?php echo e($auctionThumbImages[2]); ?>" class="opacity-50 grayscale" alt="Auction car image 3">
            </div>
        </div>
        ...
        <div class="col-span-1 lg:col-span-7">
            <div class="w-full h-[580px] overflow-hidden rounded-[80px] p-10 flex items-center justify-center bg-transparent relative group">
                <img src="<?php echo e($auctionImage); ?>" class="max-w-full max-h-full object-contain transform group-hover:scale-105 transition-transform duration-700" alt="Main auction car image">
            ...
            <div class="floating-card h-full p-8 flex flex-col justify-between">
                <div>
                    <div class="flex justify-between items-start mb-8">
                        <div>
                            <span class="text-[0.7rem] text-gray-400 font-extrabold uppercase tracking-widest block mb-2">Current Bid (<span id="stats-bid-count"><?php echo e($auction->bids_count); ?></span> Bids)</span>
                            <div class="text-4xl font-black text-[#2a3547] transition-all duration-300" id="display-price" data-price="<?php echo e($auction->current_price ?? $auction->initial_price); ?>">US $<?php echo e(number_format($auction->current_price ?? $auction->initial_price, 0)); ?></div>
                        </div>
                    ...
                    <div class="space-y-4 mb-2">
                        <div class="flex justify-between items-center py-1.5">
                            ...
                            <span class="px-3 py-1 rounded-full text-[0.7rem] font-black uppercase tracking-widest <?php echo e(($auction->car->latestInspection->overall_score ?? 0) >= 80 ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-600'); ?>">
                                <?php echo e($auction->car->latestInspection->overall_score ?? 'N/A'); ?>/100
                            </span>
                        </div>
                        <div class="flex justify-between items-center py-1.5">
                            <?php if($auction->status === 'coming_soon'): ?>
                                <span class="text-sm text-gray-400 font-semibold tracking-tight">Bidding Opens In</span>
                                <span class="text-[0.95rem] font-black text-[#ff6900] tabular-nums tracking-wider auction-timer" data-expires="<?php echo e($auction->start_at->toIso8601String()); ?>">Calculating...</span>
                            <?php else: ?>
                                <span class="text-sm text-gray-400 font-semibold tracking-tight">Time Remaining</span>
                                <span class="text-[0.95rem] font-black text-[#f44336] tabular-nums tracking-wider auction-timer" data-expires="<?php echo e($auction->end_at->toIso8601String()); ?>">Calculating...</span>
                            <?php endif; ?>
                        </div>
                ...
                <div class="mt-8 bg-gray-50/50 rounded-lg p-6 border border-dashed border-gray-100">
                    ...
                    <div class="space-y-3 max-h-[200px] overflow-y-auto pr-2 custom-scroll" id="global-bid-feed">
                        <?php $__empty_1 = true; $__currentLoopData = $auction->bids->take(5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $bid): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="flex justify-between items-center bg-white p-3 rounded-md shadow-sm border border-black/5">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-[0.7rem] font-black text-[#111827]"><?php echo e($bid->user->name); ?></div>
                                    <div class="text-[0.55rem] text-gray-400 font-bold tabular-nums"><?php echo e($bid->created_at->diffForHumans()); ?></div>
                                </div>
                            </div>
                            <div class="text-[0.75rem] font-black text-[#111827] tabular-nums underline decoration-[#ff4605] decoration-2 underline-offset-4">$<?php echo e(number_format($bid->amount)); ?></div>
                        </div>
                ...
                <div class="mt-12">
                    <?php if($auction->status === 'active'): ?>
                        ...
                        <form action="<?php echo e(route('auctions.placeBid', $auction)); ?>" method="POST" id="bid-form">
                            <?php echo csrf_field(); ?>
                            <?php
                                $bidIncrement = (float) ($auction->bid_increment ?? 500);
                                $nextBid = ($auction->current_price ?? $auction->initial_price) + $bidIncrement;
                            ?>
                            <input type="hidden" name="amount" value="<?php echo e($nextBid); ?>">
                            ...
                            <button type="submit" id="bid-submit-btn" class="w-full h-20 bg-[#ff4605] text-white rounded-lg font-black shadow-2xl shadow-orange-500/30 hover:scale-[1.02] active:scale-95 transition-all flex flex-col items-center justify-center gap-1 group">
                                ...
                                <div class="text-xl italic tracking-tighter leading-none mt-1">
                                    + $<span id="btn-total-increment"><?php echo e(number_format($bidIncrement)); ?></span>
                                    &nbsp;→&nbsp;Total: $<span id="btn-total-value"><?php echo e(number_format($nextBid)); ?></span>
                                </div>
                            </button>
                        </form>
                    <?php else: ?>
                        <div class="bg-slate-50 p-6 rounded-lg border border-dashed border-gray-200 text-center">
                            ...
                            <div class="text-[0.65rem] font-black text-slate-400 uppercase tracking-widest leading-relaxed">
                                ...
                                <span class="text-[#ff6900]"><?php echo e($auction->start_at->format('M d, H:i')); ?></span>
                            </div>
                        </div>
                    <?php endif; ?>
    ...
    <div class="mt-12 bg-white rounded-[40px] p-10 border border-[#f1f5f9] shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-8">
            <div class="p-6 bg-[#1d293d] text-white rounded-lg shadow-xl flex flex-col items-center justify-center">
                <div class="text-4xl font-black text-[#ff6900] mb-2"><?php echo e($auction->car->latestInspection->overall_score); ?>%</div>
            ...
            <div class="p-6 bg-white rounded-lg border border-gray-100 flex flex-col items-center">
                <div class="text-2xl font-black text-[#2a3547] mb-2"><?php echo e($auction->car->latestInspection->engine_score); ?>%</div>
                <div class="text-[0.55rem] font-black text-gray-400 uppercase tracking-widest">Engine & Gear</div>
            </div>
            <div class="p-6 bg-white rounded-lg border border-gray-100 flex flex-col items-center">
                <div class="text-2xl font-black text-[#2a3547] mb-2"><?php echo e($auction->car->latestInspection->paint_score); ?>%</div>
                <div class="text-[0.55rem] font-black text-gray-400 uppercase tracking-widest">Paint & Body</div>
            </div>
            <div class="p-6 bg-white rounded-lg border border-gray-100 flex flex-col items-center">
                <div class="text-2xl font-black text-[#2a3547] mb-2"><?php echo e($auction->car->latestInspection->interior_score); ?>%</div>
                <div class="text-[0.55rem] font-black text-gray-400 uppercase tracking-widest">Interior</div>
            </div>
            <div class="p-6 bg-white rounded-lg border border-gray-100 flex flex-col items-center">
                <div class="text-2xl font-black text-[#2a3547] mb-2"><?php echo e($auction->car->latestInspection->tires_score); ?>%</div>
        ...
        <div class="mt-10 p-8 bg-slate-50 rounded-[32px] border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
            <div class="flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm font-bold text-[#031629] leading-relaxed italic">"<?php echo e($auction->car->latestInspection->expert_summary ?? 'Technical audit cleared with no significant discrepancies.'); ?>"</p>
                </div>
            </div>
            <a href="<?php echo e(route('admin.inspections.show', $auction->car->latestInspection->id ?? 0)); ?>" class="px-8 py-4 bg-[#1d293d] text-white rounded-lg font-black text-[0.65rem] uppercase tracking-[0.2em] hover:bg-[#ff6900] transition-all shadow-xl shadow-slate-200">
                Full Anatomy Report
            </a>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let bidIncrement = <?php echo e((float)($auction->bid_increment ?? 500)); ?>; // from server
        let nextBidValue = currentPriceValue + bidIncrement;

        // Auction end_at (ISO string) — will be updated on time extension
        let auctionEndAt = <?php echo json_encode($auction->end_at?->toIso8601String(), 15, 512) ?>;
        ...
        function startAuctionSyncFallback() {
            ...
            syncFallbackTimer = setInterval(async () => {
                try {
                    const response = await fetch(<?php echo json_encode(route('auctions.sync', $auction), 512) ?>, {
                        headers: {
        ...
        if (window.Echo) {
            ...
            window.Echo.channel('auction.' + <?php echo e($auction->id); ?>)
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH F:\auction_app\resources\views/auctions/show.blade.php ENDPATH**/ ?>