...
<div class="px-1 space-y-10 pb-20" x-data="{ 
    ...
    <form id="audit-form" action="{{ route('admin.inspections.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="flex-1 p-6 lg:p-8 flex flex-col justify-center bg-white relative">
                            <div class="mb-5">
                                <h3 class="text-3xl font-bold italic tracking-tighter text-[#031629] uppercase leading-none">
                                    {{ $selectedCar->make ?? 'Unknown' }} <span class="text-[#ff6900] font-medium">{{ $selectedCar->model ?? 'Vehicle' }}</span>
                                </h3>
                            ...
                            <div class="flex items-center gap-8 border-t border-slate-50 pt-5">
                                <div class="space-y-1">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-[0.9rem] font-bold text-[#031629] italic truncate">{{ $selectedCar->location ?? 'Sharjah' }}</span>
                                    </div>
                                </div>
                                <div class="space-y-1">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-[0.9rem] font-bold text-[#031629] italic leading-none">{{ $selectedCar->plate_number ?? 'CERTIFIED-UNIT' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    @else
                        {{-- Dropdown if no car selected --}}
                    ...
                    <div class="relative w-full md:w-[35%] bg-[#1d293d] overflow-hidden flex items-center justify-center border-b md:border-b-0 md:border-r border-slate-50">
                        ...
                        <div class="absolute left-3 bottom-3 right-3 z-30">
                            <div class="bg-black/40 backdrop-blur-xl border border-white/10 rounded-xl px-3 py-2 shadow-2xl">
                                ...
                                <p class="text-[0.8rem] font-bold text-white italic tracking-tighter leading-none">#L-{{ $lead->id }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 p-6 flex flex-col justify-center bg-white relative">
                        <div class="mb-4">
                            <h3 class="text-xl font-bold italic tracking-tighter text-[#031629] uppercase leading-none truncate">
                                {{ $lead->car_details['name'] ?? 'Authorized Contact' }}
                            </h3>
                        ...
                        <div class="space-y-3 border-t border-slate-50 pt-4">
                            <div class="space-y-1">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[0.9rem] font-bold text-[#031629] italic">{{ $lead->car_details['phone'] ?? '+971 --' }}</span>
                                </div>
                            </div>
                        </div>

                        <div class="mt-5 flex items-center gap-2">
                            <a href="tel:{{ $lead->car_details['phone'] ?? '#' }}" class="flex-1 h-9 bg-[#1d293d] hover:bg-[#031629] text-white rounded-xl flex items-center justify-center gap-2 shadow-sm transition-all text-center">
                                <i data-lucide="phone-outgoing" class="w-3.5 h-3.5 text-[#ff6900]"></i>
                                <span class="text-[0.55rem] font-bold uppercase">Call</span>
                            </a>
                            <button type="button" onclick="window.location.href='https://wa.me/{{ preg_replace('/[^0-9]/', '', $lead->car_details['phone'] ?? '') }}'" class="flex-1 h-9 bg-slate-50 hover:bg-emerald-50 border border-slate-100 rounded-xl flex items-center justify-center gap-2 transition-all">
        ...
        {{-- Technical Control Points (Custom Fields organized by Sections) --}}
        @php
        ...
        @if(count($inspSections) > 0)
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 mt-10">
                @foreach($inspSections as $section)
                    @php $isHalf = ($section['width'] ?? 'full') === 'half'; @endphp
                    <div class="bg-white rounded-[1rem] shadow-sm border border-slate-100 transition-all duration-500 {{ $isHalf ? 'lg:col-span-6' : 'lg:col-span-12' }}"
                         x-data="{ isOpen: false }"
                         :class="isOpen ? 'overflow-visible' : 'overflow-hidden'">
                         
                         {{-- Compact Fluid Header --}}
                         <div class="flex items-center justify-between px-6 py-4 cursor-pointer hover:bg-slate-50/80 transition-all select-none group"
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex items-center gap-3">
                                    <h2 class="text-[0.75rem] font-bold text-[#031629] uppercase tracking-wider italic leading-none">{{ $section['title'] ?? 'Technical Control Area' }}</h2>
                                    <div class="w-1 h-1 rounded-full bg-slate-200"></div>
                                    <span class="text-[0.5rem] text-slate-400 font-bold uppercase tracking-widest opacity-60">{{ count($section['fields']) }} points</span>
                                </div>
                            </div>
                        ...
                        {{-- Accordion Content - Ultra Compact & Feature-Rich --}}
                        <div x-show="isOpen" 
                             x-collapse
                            ...
                            <div class="flex flex-col gap-4 mt-4">
                                 @foreach($section['fields'] as $index => $cf)
                                     @php 
                                        $cfType = $cf['type'] ?? 'text'; 
                                        $fieldId = $cf['id'] ?? $index;
                                     ...
                                     <div class="space-y-2 p-4 rounded-2xl border border-slate-100 bg-slate-50/20 hover:bg-white hover:border-orange-100 hover:shadow-sm transition-all group/field">
                                        ...
                                        <div class="relative">
                                            @if($cfType === 'text')
                                                <input type="text" name="custom_field[{{ $fieldId }}]" 
                                                       {{ $isRequired ? 'required' : '' }}
                                                       placeholder="Tap to enter {{ $fieldLabel }}..."
                                            ...
                                            @elseif($cfType === 'textarea')
                                                <textarea name="custom_field[{{ $fieldId }}]" rows="2" 
                                                       {{ $isRequired ? 'required' : '' }}
                                            ...
                                            @elseif($cfType === 'image')
                                                <div class="flex items-center gap-3 p-3 bg-white border border-dashed border-slate-300 rounded-xl shadow-sm hover:border-[#ff6900] transition-all cursor-pointer relative overflow-hidden">
                                                    ...
                                                    <input type="file" name="custom_field_img[{{ $fieldId }}]" 
                                                           accept="image/*"
                                                           {{ $isRequired ? 'required' : '' }}
                                                           class="absolute inset-0 opacity-0 cursor-pointer">
                                                </div>
                                            
                                            @elseif($cfType === 'checkbox')
                                                <label class="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-xl shadow-sm cursor-pointer hover:border-emerald-200 transition-all select-none group/tap h-11">
                                                    <input type="checkbox" name="custom_field[{{ $fieldId }}]" value="1"
                                                ...
                                                <div class="p-2.5 bg-white border border-slate-200 rounded-xl shadow-sm flex flex-wrap gap-2">
                                                    @foreach($options as $optIndex => $opt)
                                                        <label class="inline-flex items-center gap-3 px-3 py-2 bg-slate-50/50 rounded-lg text-xs font-bold text-slate-700 cursor-pointer hover:bg-[#ff6900]/5 hover:text-[#ff6900] hover:border-[#ff6900]/20 border border-transparent transition-all">
                                                            <input type="radio" name="custom_field[{{ $fieldId }}]" value="{{ $opt }}" class="w-3.5 h-3.5 accent-[#ff6900]">
                                                            <span>{{ $opt }}</span>
                                                        </label>
                                                    @endforeach
                                                </div>

                                            @elseif($cfType === 'dropdown')
                                                <div class="relative">
                                                    <select name="custom_field[{{ $fieldId }}]" 
                                                            {{ $isRequired ? 'required' : '' }}
                                            ...
                                            @elseif($cfType === 'multi_checkbox')
                                                <div class="p-2.5 bg-white border border-slate-200 rounded-xl shadow-sm flex flex-wrap gap-2">
                                                    @foreach($options as $optIndex => $opt)
                                                        <label class="inline-flex items-center gap-3 px-3 py-2 bg-slate-50/50 rounded-lg text-xs font-bold text-slate-700 cursor-pointer hover:bg-[#ff6900]/5 hover:text-[#ff6900] hover:border-[#ff6900]/20 border border-transparent transition-all">
                                                            <input type="checkbox" name="custom_field[{{ $fieldId }}][]" value="{{ $opt }}" class="w-3.5 h-3.5 rounded accent-[#ff6900]">
                                                            <span>{{ $opt }}</span>
                                                        </label>
                                                    @endforeach
                                                </div>

                                            @elseif($cfType === 'multi_select' || $cfType === 'dropdown_multi')
                                                {{-- Elite Alpine Multi-Select Dropdown --}}
                                                <div x-data="{ 
                                                        open: false, 
                                                        selected: [],
                                                        options: @js($options),
                                                    ...
                                                    <template x-for="item in selected" :key="item + '_val'">
                                                        <input type="hidden" name="custom_field[{{ $fieldId }}][]" :value="item">
                                                    </template>
                                        ...
                                        {{-- Extra Features: Attachments & Notes --}}
                                        @if($allowAttachment && $cfType !== 'image')
                                            <div class="mt-3 flex items-center gap-3">
                                                ...
                                                <input type="file" name="custom_field_attach[{{ $fieldId }}]" 
                                        ...
                                        @if($allowNotes)
                                            <div class="mt-3 relative group/note">
                                                ...
                                                <textarea name="custom_field_notes[{{ $fieldId }}]" rows="1" 
