...
<div x-data="employeeManager()" x-init="initComponent()">
    <x-admin-page-standard 
        ...
        title="Employee" 
        highlight="Manager" 
        subtitle="Manage end-to-end employee lifecycle and corporate access"
        dot="orange">
        ...
        <x-admin-data-view 
            alpine 
            alpineCount="employees.length" 
            emptyTitle="No Employees Found" 
            emptySubtitle="The workforce roster is currently dormant. Create your first employee record to begin operations." 
            emptyIcon="users">
    ...
    <x-admin-modal x-show="opened" title="Employee Details" icon="users" size="max-w-6xl">
        <div class="col-span-12 grid grid-cols-12 gap-0 overflow-visible rounded-2xl">
            ...
            <div class="col-span-12 lg:col-span-4 p-6 space-y-5 bg-slate-50/50 border-r border-slate-200">
                ...
                <div class="space-y-4 pt-2">
                    <x-elite-select label="Department" icon="building-2" x-model="form.department_id" required>
                    ...
                    <x-elite-select label="Position" icon="briefcase" x-model="form.position_id" required>
            ...
            <div class="col-span-12 lg:col-span-8 p-6 space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <x-admin-input label="Email" type="email" x-model="form.email" placeholder="email@company.com" icon="mail" />
                    <x-admin-input label="Phone" type="text" x-model="form.phone" placeholder="+971..." icon="phone" />
                    <x-admin-input label="Employee ID" type="text" x-model="form.employee_id" placeholder="EMP-001" icon="id-card" />
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <x-admin-input label="Salary" type="number" x-model="form.salary" placeholder="0.00" icon="wallet" />
                    <x-elite-date dateName="hire_date" label="Hire Date" />
                    <x-elite-date dateName="birth_date" label="Birth Date" />
                </div>
...
<script>
    function employeeManager() {
        return {
            ...
            async fetchData() {
                let url = `{{ route('admin.hr.employees.index') }}?search=${this.search}`;
