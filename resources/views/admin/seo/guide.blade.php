...
@section('title', 'How it Works - SEO Intelligence')
...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-10 flex items-center justify-between">
        ...
        <div class="flex gap-3">
             <a href="{{ route('admin.seo.dashboard') }}" class="px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-[0.7rem] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-all shadow-sm">
                Go to Dashboard
            </a>
        </div>
    </div>
                            ...
                            <div class="space-y-3 opacity-90">
                                <div class="flex gap-4">
                                    ...
                                    <span class="text-emerald-400">
                                        &lt;title&gt;<span class="text-white font-bold">{{ $page?->seo_title ?? '...Auto Generated...' }}</span>&lt;/title&gt;
                                    </span>
                                </div>
                                <div class="flex gap-4">
                                    <span class="text-slate-500">02</span>
                                    <span class="text-orange-400">
                                        &lt;meta name="description" content="<span class="text-white">{{ $page?->seo_description ?? 'AI Written Description...' }}</span>" /&gt;
                                    </span>
                                </div>
                                <div class="flex gap-4">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-[0.6rem] text-slate-400 font-bold uppercase mt-0.5">{{ url('/sitemap.xml') }}</p>
                                </div>
                            </div>
                            <a href="{{ url('/sitemap.xml') }}" target="_blank" class="px-4 py-2 bg-slate-900 text-white rounded-xl text-[0.65rem] font-black uppercase tracking-widest hover:bg-slate-800 transition-all">
                                View Sitemap
                            </a>
                        </div>
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach(['Toyota Land Cruiser Dubai', 'Used Luxury Cars UAE', 'Sell Car Instant Cash', 'Dubai Car Auctions', 'Buy Motors Sharjah'] as $k)
                    <span class="px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-lg text-[0.65rem] font-black italic text-slate-500">#{{ $k }}</span>
                    @endforeach
                </div>
...
<script>
    async function runSandboxAudit() {
        ...
        try {
            const response = await fetch('{{ route("admin.seo.analyze") }}', {
                method: 'POST',
                headers: {
