...
<div class="max-w-6xl mx-auto">
    <header class="mb-8">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.seo.dashboard') }}" class="px-4 py-2 bg-gray-600 text-white rounded-lg font-medium hover:bg-gray-700 transition-colors">
                <i class="fas fa-arrow-left mr-2"></i> Back to Dashboard
            </a>
        </div>
    </header>

    <form method="POST" action="{{ route('admin.seo.settings.update') }}" class="space-y-8">
        @csrf
        ...
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            ...
            <pre class="text-xs text-yellow-700 overflow-x-auto">{{ print_r($settings->toArray(), true) }}</pre>
        </div>
        @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <select name="agent_router_provider" class="w-full border border-gray-300 rounded-lg px-3 py-2 bg-white">
                        <option value="openai-compatible" {{ ($settings->agent_router_provider ?? 'openai-compatible') === 'openai-compatible' ? 'selected' : '' }}>OpenAI Compatible</option>
                        <option value="openai" {{ ($settings->agent_router_provider ?? '') === 'openai' ? 'selected' : '' }}>OpenAI Direct</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="agent_router_api_key" value="{{ $settings->agent_router_api_key }}" 
                ...
                <div>
                    ...
                    <input type="text" name="agent_router_base_url" value="{{ $settings->agent_router_base_url }}" 
                ...
                <div>
                    ...
                    <select name="agent_router_model" class="w-full border border-gray-300 rounded-lg px-3 py-2 bg-white">
                        @foreach(config('ai_seo.agent_router.supported_models', []) as $modelValue => $modelLabel)
                            <option value="{{ $modelValue }}" {{ ($settings->agent_router_model ?? config('ai_seo.agent_router.model')) === $modelValue ? 'selected' : '' }}>
                                {{ $modelLabel }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white rounded-md border border-gray-200 p-6">
            ...
            <div>
                ...
                <input type="password" name="bing_webmaster_api_key" value="{{ $settings->bing_webmaster_api_key }}" 
                       placeholder="Enter API key" class="w-full border border-gray-300 rounded-lg px-3 py-2">
        ...
        <div class="bg-white rounded-md border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="alert_threshold" value="{{ $settings->alert_threshold }}" 
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" name="daily_reports" value="1" {{ $settings->daily_reports ? 'checked' : '' }} 
                               class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
        ...
        <div class="flex justify-end space-x-4">
            <a href="{{ route('admin.seo.dashboard') }}" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg font-medium hover:bg-gray-300 transition-colors">
                Cancel
            </a>
